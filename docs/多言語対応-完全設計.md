...
### Phase 2: データベーススキーマ変更（完了✅ 2025-11-25）
- [x] マイグレーションSQL作成（`scripts/phase2-i18n-migration-fixed.sql`）
- [x] テスト環境でマイグレーション実行
- [x] cards, packs に JSONB カラム追加
- [x] deck_pages, info_pages に 19 カラム追加（17→19に増加）
- [x] info_article_blocks にカラム追加（info_articlesはテスト環境に存在せずスキップ）
- [x] tournaments, 通知テーブルにカラム追加
- [x] decks, trade_posts, user_collages にカラム追加
- [x] translation_cache, translation_jobs テーブル作成
- [x] インデックス作成（26個: 18 GIN + 8 補助）
- [x] 既存データの初期変換（日本語データ→JSONB形式）
- [x] **追加**: col_3, col_4, col_5 メタデータ更新（2,259件）
- [x] **追加**: CSVからカードデータインポートスクリプト作成（`scripts/import-multilingual-cards.js`）
- [x] **追加**: 6言語カードデータインポート実行（2,248件: en, ko, zh-TW, fr, es, de）
- [x] **追加**: マイグレーション実行ガイド作成（`docs/phase2-migration-guide.md`）
- [x] **追加**: カードメタデータ更新ガイド作成（`docs/cards-update-guide.md`）
- [x] **追加**: ロールバックSQL作成（`scripts/phase2-i18n-rollback.sql`）

### Phase 3: API実装（完了✅ 2025-11-25）
- [x] `lib/i18n-helpers.ts` 作成（JSONB抽出ヘルパー）
- [x] カード取得API（JSONB方式） - `/api/cards`, `/api/cards/[id]`
- [x] 記事取得API（JSONB方式） - `/api/info/[id]`
- [x] デッキガイドページ取得API（JSONB方式） - `/api/deck-pages/[id]`
- [x] ユーザーデッキ取得API（JSONB方式） - `/api/decks/[id]`
- [x] トレード投稿取得API（JSONB方式） - `/api/trades`, `/api/trades/[id]`
- [x] フォールバック実装（ja → en → デフォルト値）
- [x] エラーハンドリング（統一されたエラーレスポンス）
- [x] 言語検出機能（locale パラメータ・Accept-Language ヘッダー）
- [x] **追加**: API実装ガイド作成（`docs/phase3-api-implementation.md`）

### Phase 4: 翻訳データ投入（3週間）
- [ ] Google Cloud Translate API 統合
- [ ] Google Cloud認証設定
- [ ] `lib/translation.ts` 作成
- [x] **カード名辞書作成（6言語対応完了: en, ko, zh-TW, fr, es, de）**
- [x] **CSVからカードデータインポートスクリプト作成**（`scripts/import-multilingual-cards.js`）
- [x] **CSVデータをJSONBフォーマットに変換**（en, ko, zh-TW, fr, es, de）
- [x] **カードデータをDBに一括インポート**（2,248件）
- [ ] 不足言語（it, pt-br）のカード名翻訳（必要に応じて）
- [ ] パックデータ翻訳スクリプト
- [ ] システムコンテンツ翻訳スクリプト
- [ ] バックグラウンド翻訳ワーカー実装
- [ ] translation_jobs 処理ロジック
- [ ] 翻訳キャッシュ実装
- [ ] コメント翻訳時のカード名置換機能実装

### Phase 5: フロントエンド実装（2週間）
- [ ] カード表示コンポーネント更新
- [ ] 記事表示コンポーネント更新
- [ ] デッキ表示コンポーネント更新
- [ ] トレード投稿表示コンポーネント更新
...
**オプションA: 投稿時言語のみ保存（推奨 Phase 1）**

投稿時の言語で保存し、他言語ユーザーには翻訳機能を提供
...
INSERT INTO cards_i18n (card_id, language, name, description) VALUES
  (1, 'en', 'Pikachu', 'When this Pokémon becomes your Active Pokémon...'),
  (2, 'en', 'Charizard', 'Discard 2 Energy from this Pokémon...');
...
**⚠️ 重要**: 最新の設計では**JSONB方式**を採用しています（`docs/多言語スキーマ変更サマリー.md`参照）

### 完全なマイグレーションSQL

```sql
-- ===================================
-- Phase 1: マスターデータ（cards, packs）
-- ===================================
...
-- ===================================
-- Phase 2: 管理者コンテンツ（17カラム×2テーブル + 記事）
-- ===================================

-- 3. deck_pages: デッキガイド（17カラム）
...
-- 4. info_pages: 記事ページ（deck_pagesと同じ17カラム）
...
-- 6. info_article_blocks: 記事ブロック（1カラム）
...
2. サービスアカウント作成または選択
3. キーを作成 > JSON > ダウンロード
4. Vercelにアップロード（Base64エンコード）

#### 4-1. カードデータの翻訳（JSONB方式）
...
```typescript
// 入力（日本語コメント）
const originalComment = "ピカチュウexとミュウツーexのデッキが強いです";
...
// ↑ この例ではたまたま一致しているが、通常は翻訳がずれる

// より正確な例:
// 入力: "リザードンexが欲しいです"
// 翻訳前: "I want a Lizard ex"  ← 誤訳
// 置換後: "I want a Charizard ex" ← 正しいカード名に置換
```
...
### バックエンド
- **Supabase**: データベース
- **PostgreSQL**: JSONB型で多言語データ管理

### 翻訳API
- **Google Cloud Translate API**: メイン翻訳サービス
  - リアルタイム翻訳
  - 100言語以上対応
  - 文字数課金（$20/100万文字）
  - 既存のGoogle Cloud設定を活用
...
```bash
# 1. バックアップ作成
pg_dump "本番のPOSTGRES_URL" -f backup_before_i18n_migration.sql

# 2. マイグレーション実行
psql "本番のPOSTGRES_URL" < migrations/001_add_i18n_tables.sql

# 3. 確認
psql "本番のPOSTGRES_URL" -c "\dt *i18n*"

# 4. データ投入
SUPABASE_URL=本番URL SUPABASE_SERVICE_ROLE_KEY=本番KEY pnpm tsx scripts/migrate-i18n-data.ts
```
...
1. **ユーザー設定** (user_profiles.preferred_language)
2. **URLパラメータ** (/en/trades)
...
## 📊 Phase 2 完了サマリー（2025-11-25）

### 実施内容

**1. データベーススキーマ変更**
- 13テーブルに48カラムの多言語対応JSONBカラムを追加
- 26個のインデックスを作成（18 GIN + 8 BTREE）
- translation_cache, translation_jobs 補助テーブルを作成
- テスト環境: `db.oflucnwezzqqtxryonhf.supabase.co`
...
- 更新レコード数:
  ...
  - packs: 18件
  - deck_pages: 8件
  - info_article_blocks: 9件
  - tournaments: 3件
  - decks: 2件
  - trade_posts: 149,862件
  - user_collages: 1件

**3. 多言語カードデータインポート**
- CSVから6言語のカードデータをインポート（2,248件）
- 対応言語: 英語(en), 韓国語(ko), 中国語繁体字(zh-TW), フランス語(fr), スペイン語(es), ドイツ語(de)
- カバー率: 99.5%（2,246/2,257件）

**4. スクリプト・ドキュメント作成**
- `scripts/phase2-i18n-migration-fixed.sql` - マイグレーションSQL
- `scripts/phase2-i18n-rollback.sql` - ロールバックSQL
- `scripts/generate-cards-update-sql.js` - カードメタデータ更新SQL生成
- `scripts/update-cards-metadata.sql` - カードメタデータ更新SQL（2,259件）
- `scripts/import-multilingual-cards.js` - 多言語カードデータインポートスクリプト
- `scripts/import-multilingual-cards.sql` - 生成されたインポートSQL（2,248件）
- `docs/phase2-migration-guide.md` - マイグレーション実行ガイド
- `docs/cards-update-guide.md` - カードメタデータ更新ガイド
- `docs/current-database-schema.md` - 現在のデータベーススキーマドキュメント
