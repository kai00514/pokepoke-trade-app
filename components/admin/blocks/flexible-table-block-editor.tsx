...
export function FlexibleTableBlockEditor({ data, onChange }: FlexibleTableBlockEditorProps) {
  // データの初期化
  const safeData = data || { columns: [], rows: [], style: "default" }
  ...
  const updateColumn = (columnId: string, updates: Partial<(typeof safeData.columns)[0]>) => {
    onChange({
      ...safeData,
      columns: safeData.columns.map((col) => (col.id === columnId ? { ...col, ...updates } : col)),
    })
  }
  ...
  const deleteColumn = (columnId: string) => {
    onChange({
      ...safeData,
      columns: safeData.columns.filter((col) => col.id !== columnId),
      rows: safeData.rows.map((row) => {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Select value={safeData.style} onValueChange={(value: any) => onChange({ ...safeData, style: value })}>
