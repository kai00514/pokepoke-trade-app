...
export async function generateCollageImageBuffer(params: GenerateCollageImageParams): Promise<Buffer> {
  ...
  const title1TextSvg = `<svg width="${CANVAS_WIDTH}" height="${CANVAS_HEIGHT}">
    <text x="${CANVAS_WIDTH / 2}" y="${title1Y + 45}"
          font-family="sans-serif" font-size="48" font-weight="bold"
          fill="white" text-anchor="middle"
          stroke="rgba(0,0,0,0.3)" stroke-width="2">${title1}</text>
  </svg>`
  const title2TextSvg = `<svg width="${CANVAS_WIDTH}" height="${CANVAS_HEIGHT}">
    <text x="${CANVAS_WIDTH / 2}" y="${title2Y + 45}"
          font-family="sans-serif" font-size="48" font-weight="bold"
          fill="white" text-anchor="middle"
          stroke="rgba(0,0,0,0.3)" stroke-width="2">${title2}</text>
  </svg>`
  const footerTextSvg = `<svg width="${CANVAS_WIDTH}" height="${CANVAS_HEIGHT}">
    <text x="${CANVAS_WIDTH / 2}" y="${CANVAS_HEIGHT - 12}"
          font-family="sans-serif" font-size="24"
          fill="rgba(0,0,0,0.6)" text-anchor="middle">ポケポケコラージュ画像メーカー@PokeLink</text>
  </svg>`
